...
class MexcClient(BaseRestClient):
    ...
    def _sign(self, timestamp: int, params_json: str) -> str:
        message = f"{self.api_key}{timestamp}{params_json}"
        logger.info(f"MEXC sign: api_key_len={len(self.api_key)}, timestamp={timestamp}, params_json={params_json[:100]}...")
        logger.info(f"MEXC sign: secret_key first 5 chars: {self.secret_key[:5] if self.secret_key else 'None'}")
        signature = hmac.new(
        ...
        logger.info(f"MEXC signature result: {sig_b64}")
        return sig_b64

    def _headers(self, path: str, body_json: str = "") -> Dict[str, str]:
